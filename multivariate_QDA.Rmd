---
title: "Multivariate Quadratic Discriminant Analysis"
output: 
    html_document:
        toc: TRUE
        toc_float: TRUE
...
This example walks through using quadratic discriminant analysis to classify observations in a three-class multivariate setting with (idealized) generated data. I'd suggest reading the [multivariate LDA](multivariate_LDA.html){target="_blank"} example first.
...
Just like in the [univariate LDA](univariate_LDA.html){target="_blank"} and [multivariate LDA](multivariate_LDA.html){target="_blank"} examples, we will use the Bayes classifier and its decision boundaries to compare our model against since discriminant analysis is the same as applying the Bayes classifier but with sample estimates.
